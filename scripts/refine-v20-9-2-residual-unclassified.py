...
RULES=[
    ...
    ('car-accessories','automotive',r'\b(?:car\s+(?:seat\s+gap\s+filler|neck\s+pillow|headrest\s+pillow|armrest|sun\s+visor|windshield\s+cover|wash\s+brush|wash\s+mitt|drying\s+towel|air\s+freshener|key\s+cover|key\s+case|door\s+protector|bumper\s+protector|mirror\s+cover)|seat\s+belt\s+(?:cover|pad|extender))\b',r'\b(?:toy|model|miniature)\b'),
    ...
    ('pet-supplies','pets',r'\b(?:(?:pet|dog|cat)\s+(?:carrier|travel\s+carrier|backpack\s+carrier|clothes|clothing|sweater|jacket|costume|shoes|boots|stairs|ramp|muzzle|gate)|(?:carrier|travel\s+carrier|stairs|ramp|muzzle)\s+for\s+(?:dogs?|cats?|pets?)|dog\s+waste\s+bags?|pet\s+waste\s+bags?)\b',''),
    ...
    ('lighting','lighting',r'\b(?:string\s+lights?|fairy\s+lights?|rope\s+lights?|strip\s+lights?|led\s+lamp|usb\s+lamp|rechargeable\s+lamp|closet\s+light|wardrobe\s+light|cabinet\s+light|puck\s+lights?)\b',r'\b(?:car|vehicle|automotive|bike|bicycle)\b'),
    ...
    ('food-storage','kitchen',r'\b(?:food\s+storage\s+containers?|meal\s+prep\s+containers?|bento\s+box|lunch\s+box|lunchbox|reusable\s+food\s+bags?|food\s+container\s+set)\b',''),
    ('home-organization','home',r'\b(?:kitchen\s+organizer|spice\s+rack|dish\s+rack|dish\s+drainer|sink\s+caddy|utensil\s+holder|cutlery\s+tray|pan\s+organizer|pot\s+rack|cabinet\s+organizer)\b',''),
    ...
    ('hair-care','beauty',r'\b(?:hair\s+brush|hair\s+comb|hair\s+clips?|hair\s+claw|hair\s+pins?|hair\s+bands?|hair\s+ties?|hair\s+accessor(?:y|ies)|scrunch(?:y|ie|ies)|hair\s+bonnet)\b',r'\b(?:pet|dog|cat|carpet)\b'),
