...
FOOT_NEG=re.compile(
    ...
    r'\b(?:boot cut|bootcut|baggy jeans|straight jeans|skinny jeans|denim jeans)\b|'
    r'\b(?:bunion reliever|bunion support|toe separator|toe corrector|foot corrector|arch support insert|shoe insert)\b|'
    r'\b(?:labubu|doll clothes|doll shoes|toy shoes|dress up accessories)\b|'
    r'\b(?:pet shoes?|pet protective shoes?|dog boots?|dog shoes?|dog booties?|cat dog boots?|pet supplies)\b|'
    r'\b(?:pet house|pet bed|slipper shaped|semi-enclosed pet)\b|'
    r'\b(?:cleaning cream|leather care|shoe cleaning|shoe cleaner|shoe polish)\b|'
...
RELEASE_BAD=re.compile(r'\b(?:cosplay|costume|cold shoe|hot shoe|boot cut|bootcut|bottle opener|wine rack|shoe pendant|shoe charm|anti-clog|clog remover|snow blower|skid plate|brake shoe|phone case|airpods|shoe machine|shoe molding|flooring installation|epoxy shoe|medical boot|walking boot brace|bunion reliever|bunion support|labubu|doll shoes|pet shoes|dog boots|pet bed|cleaning cream|shoe cleaner|baggy jeans|straight jeans|christmas|xmas)\b',re.I)
...
def rank(row): return (int(row.get('r') or 0),1 if row.get('x') else 0,1 if clean(row.get('im')) else 0,1 if row.get('p') else 0,-len(title(row)))
...
foot=defaultdict(list); browse=defaultdict(list); seen_ids=set(); total=0
...
def payload(source,per_seller,kind):
    ...
    return {'version':'21.13.7','kind':kind,'generated_from':'data/v20-9/products/*.json','records_scanned':total,'seller_count':len(sellers),'packed_record_count':len(records),'counts':counts,'sellers':sellers,'records':records}
...
if bad_packed: raise SystemExit('Release guard rejected footwear false positives: '+repr(bad_packed[:20]))
