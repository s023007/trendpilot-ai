...
def payload(source,per_seller,kind):
    ...
    return {'version':'21.13.4','kind':kind,'generated_from':'data/v20-9/products/*.json','records_scanned':total,'seller_count':len(sellers),'packed_record_count':len(records),'counts':counts,'sellers':sellers,'records':records}
...
required={'AliExpress','TikTok Shop US'}
missing=required-set(foot_payload['sellers'])
if missing: raise SystemExit('Strict footwear index lost known genuine footwear sellers: '+', '.join(sorted(missing)))
