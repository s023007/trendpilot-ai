(() => {
  ...
  function couponScore(c,direct){const end=c.end_at?new Date(c.end_at).getTime():Infinity,days=(end-Date.now())/86400000;let s=Number(c.priority_score||0)+(direct?80:0)+(c.code&&!/not required|не нужен/i.test(c.code)?12:0);if(state.sort==='ending'&&Number.isFinite(days)&&days>=0)s+=Math.max(0,80-days);return s}
  function couponRows(){
