(() => {
  ...
  const state={q:'',cat:'all',shownDeals:12,shownCoupons:8,sort:'best'};

  function flattenProducts(){
    const src=window.TRENDPILOT_MATCHED_PRODUCTS||{}; const out=[],seen=new Set();
  ...
  function productBlob(p){return low([p.name,p.category,p.advertiser,p.programme,p._group].join(' '))}
  function couponBlob(c){return low([c.title,c.description,c.merchant_name,c.code,(c.categories||[]).join(' '),(c.types||[]).join(' ')].join(' '))}
  function terms(q){return low(q).split(/\s+/).filter(x=>x.length>1)}
  function textMatch(blob,q){const ts=terms(q);return !ts.length||ts.every(t=>blob.includes(t))}
  function categoryMatch(blob){const c=CATS[state.cat]||CATS.all;return state.cat==='all'||c.re.test(blob)}
  function dealScore(p){
    const exact=textMatch(productBlob(p),state.q)?70:0;
    return exact+(p._disc||0)*1.5+Number(p.offerQuality||0)*.25+Number(p.matchScore||0)*.15+(p.publicationValidation?.status==='verified-exact-product'?12:0);
  }
  function dealRows(){
    let rows=products.filter(p=>categoryMatch(productBlob(p))&&textMatch(productBlob(p),state.q));
    if(state.sort==='discount')rows.sort((a,b)=>b._disc-a._disc||dealScore(b)-dealScore(a));
    else if(state.sort==='low')rows.sort((a,b)=>(a._price||1e9)-(b._price||1e9));
    else rows.sort((a,b)=>dealScore(b)-dealScore(a));
    return rows;
  }
  ...
  function couponScore(c,direct){
    const end=c.end_at?new Date(c.end_at).getTime():Infinity,days=(end-Date.now())/86400000;
    let s=Number(c.priority_score||0)+(direct?80:0)+(c.code&& !/not required|не нужен/i.test(c.code)?12:0);
    if(state.sort==='ending'&&Number.isFinite(days)&&days>=0)s+=Math.max(0,80-days);
    return s;
  }
  function couponRows(){
    const base=coupons.filter(c=>countryOk(c)&&langOk(c));
    const q=state.q; const cat=state.cat;
    const direct=base.filter(c=>textMatch(couponBlob(c),q)&&categoryMatch(couponBlob(c))).map(c=>({...c,_direct:true}));
    if(!q&&cat==='all')return direct.sort((a,b)=>couponScore(b,true)-couponScore(a,true));
    const merchants=new Set(dealRows().slice(0,80).map(p=>clean(p.advertiser)).filter(Boolean));
    ...
    return [...direct,...fallback].sort((a,b)=>couponScore(b,a._direct)-couponScore(a,b._direct));
  }
  function dealCard(p){
    const seller=clean(p.advertiser||p.seller||'Seller'),price=money(p._price,p.currency),old=p._old>p._price?money(p._old,p.currency):'';
    const disc=p._disc>0?`${Math.round(p._disc)}% saving`:'Current price';
    return `<article class="tp214-deal"><div class="tp214-media">${p.image?`<img loading="lazy" src="${esc(p.image)}" alt="">`:''}<span class="tp214-badge">${esc(disc)}</span></div><div class="tp214-body"><div class="tp214-meta"><strong>${esc(seller)}</strong><span>${esc(p.category||'Product')}</span></div><h3>${esc(p.name)}</h3><div class="tp214-price"><b>${esc(price||'Check price')}</b>${old?`<s>${esc(old)}</s>`:''}</div><div class="tp214-actions"><a class="primary" href="/deal/?id=${encodeURIComponent(p._key)}">View deal</a><a href="/compare/?add=${encodeURIComponent(p.id||p._key)}">Compare</a></div></div></article>`;
  }
  function couponCard(c){
    const realCode=clean(c.code)&&!/not required|не нужен/i.test(clean(c.code));
    const discount=clean(c.discount?.text)||clean(c.title).match(/(?:\$|€|£)?\s*\d+(?:\.\d+)?\s*%?/i)?.[0]||'Offer';
    const min=Number(c.minimum_order?.value)>0?`Minimum ${money(c.minimum_order.value,c.minimum_order.currency||'USD')}`:'No minimum shown';
    const end=fmtDate(c.end_at); const hint=c._direct?'Matches your search':'Marketplace coupon — check eligibility';
    return `<article class="tp214-coupon"><div class="tp214-coupon-top"><strong>${esc(c.merchant_name)}</strong><span>${esc(hint)}</span></div><h3>${esc(discount)} — ${esc(c.title)}</h3><p>${esc(min)}${end?` · Ends ${esc(end)}`:''}</p><div class="tp214-code"><code>${esc(realCode?c.code:'No code needed')}</code>${realCode?`<button type="button" data-copy-code="${esc(c.code)}">Copy</button>`:''}</div><div class="tp214-actions"><a class="primary" href="/coupon/?id=${encodeURIComponent(c.id)}">View coupon</a></div><small class="tp214-note">Terms, country eligibility and final price must be confirmed at checkout.</small></article>`;
  }
  function render(){
    const dr=dealRows(),cr=couponRows(),dg=$('[data-tp-deal-products]'),cg=$('[data-tp-coupon-grid]');
    if(dg)dg.innerHTML=dr.length?`<div class="tp214-grid">${dr.slice(0,state.shownDeals).map(dealCard).join('')}</div>`:`<div class="tp214-empty"><b>No dedicated deal card found for “${esc(state.q||CATS[state.cat]?.label||'this search')}”.</b><p>We can still search the full product catalogue.</p><a class="tp-btn tp-btn-primary" href="/find/?q=${encodeURIComponent(state.q||CATS[state.cat]?.label||'popular products')}">Search all products</a></div>`;
    ...
    if(cg)cg.innerHTML=cr.length?`<div class="tp214-coupon-grid">${cr.slice(0,state.shownCoupons).map(couponCard).join('')}</div>`:`<div class="tp214-empty"><b>No dedicated coupon matches this product yet.</b><p>Try All countries / All languages, or check the product deals above.</p></div>`;
    ...
    const s1=$('[data-stat-deals]'),s2=$('[data-stat-coupons]'),s3=$('[data-stat-sellers]');if(s1)s1.textContent=String(dr.length);if(s2)s2.textContent=String(cr.length);if(s3)s3.textContent=String(new Set([...dr.map(x=>clean(x.advertiser)),...cr.map(x=>clean(x.merchant_name))].filter(Boolean)).size);
  ...
  function bind(){
    ...
    $('[data-coupon-country]')?.addEventListener('change',()=>{state.shownCoupons=8;render()});
    $('[data-coupon-language]')?.addEventListener('change',()=>{state.shownCoupons=8;render()});
    $('[data-deals-more]')?.addEventListener('click',()=>{state.shownDeals+=12;render()});
    $('[data-coupon-toggle]')?.addEventListener('click',()=>{state.shownCoupons+=12;render()});
