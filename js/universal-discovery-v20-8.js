(() => {
  "use strict";
  const V="20.8.4",d=document,$=(s,r=d)=>r.querySelector(s),C=v=>String(v??"").replace(/\s+/g," ").trim(),L=v=>C(v).toLowerCase(),E=v=>C(v).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]));
  ...
  function card(r){
    const pi=priceInfo(r),rare=rarity(r.r),rareHtml=rare?`<span class="tp80-mini-rare">${E(rare)}</span>`:"",href=r.seo||`/item/?id=${encodeURIComponent(r.id)}&q=${encodeURIComponent(q)}`,cta=r.seo?"View rare find":"View details";
    return `<article class="tp78-card tp80-universal-card"><div class="tp78-media">${r.im?`<img src="${E(r.im)}" alt="${E(r.t)}" width="360" height="360" loading="lazy">`:"<span>No image</span>"}</div><div class="tp78-body"><div class="tp78-top"><b>${E(r.b||r.tyl||"Product")}</b><span>${E(r.se)}</span></div><h3>${E(r.t)}</h3><strong class="tp78-price">${E(pi.label)}</strong><span class="tp80-price-proof ${pi.kind}">${E(pi.proof)}</span>${rareHtml}<p class="tp80-universal-meta">${E(r.tyl)} · ${E((r.ro||"main").replaceAll("_"," "))}</p><div class="tp78-actions"><a class="tp78-primary internal-detail" href="${E(href)}">${cta} →</a></div>${!r.x&&r.p?'<small class="tp80-route-note">Price came from seller catalogue data. Review the TrendPilot details page before choosing whether to open the seller.</small>':""}</div></article>`;
  }
