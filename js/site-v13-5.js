...
const rankedExact=state.products
  ...
  function filterProducts(rows) {
    ...
    let out=rows.filter(p=>{if(!tpCjPublicAllowed(p))return false;if(!tpCjPublicAllowed(p))return false;if(!tpCjPublicAllowed(p))return false;if(!tpCjPublicAllowed(p))return false;if(!tpCjPublicAllowed(p))return false;if(!tpCjPublicAllowed(p))return false;if(!tpCjPublicAllowed(p))return false;if(!tpCjPublicAllowed(p))return false;if(!tpPublicSellerAllowed(p))return false;
