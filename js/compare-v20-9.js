(() => {
  "use strict";
  const V="21.5.0",d=document,$=s=>d.querySelector(s),C=v=>String(v??"").replace(/\s+/g," ").trim(),E=v=>C(v).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]));
  ...
  const priceProof=r=>!r.p?"Check with seller":r.x?"Price for this listing":"Price from seller";
  ...
  function productCard(r){
    ...
    return `<article class="tp90-product"><a class="tp90-media" href="${E(detail)}">${r.im?`<img src="${E(r.im)}" alt="${E(title)}" width="500" height="500" loading="lazy">`:"<span>No image</span>"}</a><div class="tp90-copy"><div class="tp90-type"><span>${E(r.tyl||r.ty)}</span><span>${E(sellerName)}</span></div><h2><a href="${E(detail)}">${E(title)}</a></h2><p class="tp90-price">${r.p?E(money(r)):"Check current price"}</p><span class="tp90-proof">${E(priceProof(r))}</span><div class="tp90-facts"><div class="tp90-fact"><b>Seller</b><span>${E(sellerName)}</span></div><div class="tp90-fact"><b>Category</b><span>${E(r.tyl||r.ty||"Product")}</span></div><div class="tp90-fact"><b>Condition</b><span>${E(r.co||"Check seller")}</span></div>${code?`<div class="tp90-fact"><b>Product code</b><span>${E(code)}</span></div>`:''}</div><div class="tp90-actions">${seller}<button class="tp90-secondary" type="button" data-v209-remove="${E(r.id)}">Remove</button></div></div></article>`
  }
  ...
  async function boot(){
    ...
    if(!records.length){write([]);host.innerHTML='<section class="tp90-empty"><h2>No comparable products remain.</h2><p>Choose main products rather than cases, parts or repair items.</p><a href="/find/">Find products</a></section>';$("[data-v209-compare-count]").textContent="0 products";return}
