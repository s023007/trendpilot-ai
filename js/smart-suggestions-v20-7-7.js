(() => {
  'use strict';
  const VERSION='20.7.7',d=document;
  const clean=v=>String(v??'').replace(/\s+/g,' ').trim();
  const lower=v=>clean(v).toLowerCase();
  const esc=v=>clean(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
  const selector='input[data-tp-finder-input], .tp-search input[type="search"][name="q"]';
  const intents=[
    ['phone','Phones · browse smartphones'],['laptop','Laptops · portable computers'],
    ['headphones','Audio · headphones and earbuds'],['perfume','Fragrance · perfume listings'],
    ['power bank','Portable charging'],['smartwatch','Smart watches'],['dog food','Pet food'],
    ['air conditioner','Cooling and AC'],['3d filament','3D printing material'],['tools','Tools and workshop']
  ];
  let panel=null,input=null,seq=0,gesture=null,timer=null,controller=null;
  const localRows=q=>{const n=lower(q);return n.length<2?[]:intents.filter(([v])=>lower(v).startsWith(n)||lower(v).includes(n)).slice(0,5).map(([value,meta])=>({value,meta,kind:'intent'}));};
  ...
  async function remoteRows(q){if(controller)controller.abort();controller=new AbortController();const p=new URLSearchParams({q,limit:'8'});const seller=currentSeller();if(seller)p.set('seller',seller);const r=await fetch(`/api/products-v20-suggest?${p}`,{cache:'no-store',signal:controller.signal,headers:{accept:'application/json'}});if(!r.ok)return[];const j=await r.json();return j?.ok&&Array.isArray(j.rows)?j.rows:[];}
  async function load(q){const local=localRows(q);let remote=[];try{remote=await remoteRows(q);}catch(e){if(e?.name!=='AbortError')console.warn('[TrendPilot autocomplete]',e);}const out=[],seen=new Set();for(const row of [...local,...remote]){const key=lower(row?.value);if(!key||seen.has(key))continue;seen.add(key);out.push({...row,image:''});if(out.length>=8)break;}return out;}
  async function render(el){const token=++seq,q=clean(el.value);if(q.length<2){close();return;}const rows=await load(q);if(token!==seq)return;input=el;panel=ensure(el);panel._rows=rows;panel.querySelector('.tp-amazon-list').innerHTML=rows.map((row,i)=>`<button type="button" class="tp-amazon-row" role="option" data-i="${i}"><span class="tp-amazon-search-icon" aria-hidden="true">⌕</span><span class="tp-amazon-copy"><b>${esc(row.value)}</b>${row.meta?`<small>${esc(row.meta)}</small>`:''}</span><span class="tp-amazon-arrow" aria-hidden="true">›</span></button>`).join('');panel.hidden=!rows.length;}
