(() => {
  ...
  function tier(value,q){
    const v=lower(value),n=lower(q);
    if(!v||!n)return 99;
    if(v.startsWith(n))return 0;
    if(n.length===1)return 99;
    const words=v.split(/[^a-z0-9]+/i).filter(Boolean);
    if(words.some(w=>w.startsWith(n)))return 1;
    if(n.length>=3&&v.includes(n))return 2;
    return 99;
  }

  const localRows=q=>fallbackIntents
  ...
  async function remoteRows(q){
    ...
    const p=new URLSearchParams({q,limit:'20'}),seller=currentSeller();
