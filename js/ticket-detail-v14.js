(() => {
  ...
  async function init(){
    ...
    const title=clean(x.eventTitle||x.title),provider=clean(x.provider)||"Ticket seller",description=clean(x.description),detailFacts=facts([["Date",x.date],["Venue",usableVenue(x)],["City",x.city],["Section",x.section],["Row",x.row],["Price",money(x)]]);d.title=`${title} — TrendPilot AI`;
    host.innerHTML=`<a class="tp-ticket-v141-back" href="/tickets/">← Back to tickets</a><article class="tp-ticket-v141-detail"><span class="tp-kicker">${esc(provider)}</span><h1>${esc(title)}</h1>${description?`<p class="tp-ticket-v141-detail-lead">${esc(description)}</p>`:""}${detailFacts}<div class="tp-ticket-v141-actions"><a href="/tickets/">See other events</a><a class="primary" href="${esc(x.url)}" target="_blank" rel="nofollow sponsored noopener">Check tickets at ${esc(provider)} ↗</a></div></article>`;
  }
