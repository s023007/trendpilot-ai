(() => {
  ...
  const SCOPE_GROUPS = {
    ...
    "home": ["home-kitchen","tools"],
    "school": ["office-school"],
    "sports": ["sports-outdoors"],
    "beauty": ["beauty-care"],
    "kids": ["baby-kids","toys-games"],
    "software": ["software"],
  ...
  function makePlan(q, manifest, scope = "") {
    const audience = inferAudience(q), family = inferFamily(q);
    const scopedGroups = SCOPE_GROUPS[scope] || [];
  ...
  function resultCountLabel() {
    const found = activeProducts().length;
  ...
  function renderFinder() {
    ...
    if(status){
      ...
      status.textContent=`Showing ${fmt.format(visible.length)} of ${fmt.format(rows.length)} ${noun}. ${fmt.format(scan)} candidates checked. ${progress}`;
    }
  ...
  function renderTabs(){
    ...
    host.innerHTML=`<button class="${state.activeTab==='exact'?'is-active':''}" data-result-tab="exact" type="button">Exact matches <span>${fmt.format(state.exact.length)}${exactSuffix}</span></button><button class="${state.activeTab==='related'?'is-active':''}" data-result-tab="related" type="button">Related alternatives <span>${fmt.format(state.alternatives.length)}</span></button>`;
  }

  async function performSearch(query,push=true,scope=""){
    state.query=clean(query)||"popular products"; state.scope=scope||""; state.shown=24; state.activeTab="exact"; state.products=[]; state.exact=[]; state.alternatives=[]; state.segmentState.clear();
  ...
  function initForms(){
    $$('[data-tp-home-search],[data-tp-tool-form]').forEach(form=>form.addEventListener('submit',e=>{
      ...
      const q=parts.join(' ');if(q){const params=new URLSearchParams({q});if(scope)params.set('scope',scope);location.href=`/find/?${params.toString()}`;}
    }));
  }
  function initFinder(){
    const form=$('[data-tp-finder-form]');if(!form)return;
    form.addEventListener('submit',e=>{e.preventDefault();performSearch($('[data-tp-finder-input]')?.value||'products',true,$('[data-tp-finder-scope]')?.value||'');});
