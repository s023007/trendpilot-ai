...
function parseVariant(label,counts,stable){
  ...
  const normalized=parts.length?parts.join(" · "):t;
...
function cleanVariants(body){
  ...
  if(!section)return {body,count:0,changed:false};
  ...
  if(!rows.length)return {body,count:0,changed:false};
...
function heroTruth(body,variantCount){
  ...
  return body.replace(/<section class="hero"><div class="hero-media">([\s\S]*?)<\/div><div class="hero-copy">([\s\S]*?)<p>(\d+) active listing(s?) from (\d+) seller(s?) · \d+ configuration(s?)<\/p><\/div><\/section>/i,
    (all,media,copy,records,_pl,sellers)=>{
      ...
      const cfg=variantCount?` · ${variantCount} configuration${variantCount===1?"":"s"}`:"";
...
@media(max-width:650px){main{padding-bottom:154px!important}.hero h1{font-size:clamp(25px,6.5vw,30px)!important;line-height:1.06!important}.variant{padding:15px!important}}
`;
